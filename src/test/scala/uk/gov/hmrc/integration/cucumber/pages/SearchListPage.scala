...
object SearchListPage extends BasePage with Matchers {
  ...
  def verifyTheInputRecord(): Unit = {
    ...
    Assert.assertEquals(timestamp, scalaCells.head.getText substring(0,10))
    Assert.assertEquals(getMessage("testValue.idType"), scalaCells(1).getText)
    Assert.assertEquals("1234", scalaCells(2).getText)
    Assert.assertEquals(getMessage("testValue.idDateReceived"), scalaCells(4).getText)
    Assert.assertEquals(getMessage("testValue.idNotFoundStatus"), scalaCells(5).getText)
  }
